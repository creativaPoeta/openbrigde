...
const loadDashboardRows = cache(async function loadDashboardRows() {
  ...
  const [linksResult, eventsResult] = await Promise.all([
    client.from("short_links").select("*").order("created_at", { ascending: false }).limit(1500),
    client
      .from("link_events")
  ...
  const error = linksResult.error || eventsResult.error;
  if (error) {
    return {
      linkRows: [],
      eventRows: [],
      error: tableErrorMessage(error, "Could not load dashboard data."),
    };
  }
...
export async function listRecentLinks(limit = 12, filters = {}) {
  const { linkRows, eventRows, error } = await loadDashboardRows();
...
export async function listRecentEvents(limit = 20, filters = {}) {
  const { linkRows, eventRows, error } = await loadDashboardRows();
...
export async function getDashboardExportRows(filters = {}) {
  const { linkRows, eventRows, error } = await loadDashboardRows();
  if (error) {
    return { data: [], error };
  }
