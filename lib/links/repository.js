import { normalizeDestination } from "@/lib/links/destinations";
...
async function getLinkEventSummary(client, linkId) {
  const [
    ...
    client
      .from("link_events")
      .select("id, event_type, source_app, os, browser, in_app, created_at, slug_snapshot")
      .eq("link_id", linkId)
...
export async function createShortLink(input) {
  ...
  try {
    ...
    const slug = await getUniqueSlug(client, input.slug, input.title || destination.destinationValue);
...
export async function listRecentEvents(limit = 20) {
  ...
  const { data, error } = await client
    .from("link_events")
    .select("id, link_id, slug_snapshot, event_type, source_app, os, browser, in_app, created_at")
    .order("created_at", { ascending: false })
    .limit(limit);
